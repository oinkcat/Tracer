...
void SetTitleForOperation(bool, size_t);
...
void StartRender()
{
	...
	// Set window title
	SetTitleForOperation(true, NULL);
}
...
void WatchTimerProc(HWND hWnd, UINT msg, UINT_PTR lPtr, DWORD wPtr)
{
	...
	if (numRunningThreads == 0)
	{
		...
		// Post finish message
		size_t ticksElapsed = GetTickCount64() - startTick;
		PostMessage(hWnd, WM_RENDERED, ticksElapsed, 0);
	}
}

void SetTitleForOperation(bool rendering, size_t frameTime)
{
	WCHAR titleBuff[32];

	if (rendering)
	{
		wsprintf(titleBuff, L"%s - [rendering]", szTitle);
	}
	else
	{
		wsprintf(titleBuff, L"%s - [frame time: %d]", szTitle, (int)frameTime);
	}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case IDM_REPEAT:
			StartRender();
			EnableMenuItem(GetMenu(hWnd), IDM_REPEAT, MF_DISABLED);
			break;
		case IDM_EXIT:
			DestroyWindow(hWnd);
			break;
	...
	case WM_DESTROY:
		...
		break;
	case WM_RENDERED:
		EnableMenuItem(GetMenu(hWnd), IDM_REPEAT, MF_ENABLED);
		SetTitleForOperation(false, (size_t)wParam);
		break;
	default:
		return DefWindowProc(hWnd, message, wParam, lParam);
	}
